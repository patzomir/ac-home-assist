...
1. **Смарт контакт (Nous A1Z или Sonoff S26R2ZB)** — мери реални kWh, праща данните към хъба по Zigbee
2. **Хъб (ESP32-C6 Zigbee координатор) + IR емитер (ESP32-H2)** — получава данните от контакта, управлява климатика по разписание чрез IR
...
| Zigbee смарт контакт (Nous A1Z / Sonoff S26R2ZB) | ~29 BGN | По 1 на климатик |
...
|---|---|---|---|---|
| 1 AC | ~65 BGN | ~130 BGN | ~115 BGN | ~1 сезон |
| 2 AC | ~103 BGN | ~205 BGN | ~230 BGN | <1 сезон |
| 3 AC | ~142 BGN | ~285 BGN | ~345 BGN | <1 сезон |

Хъбът е споделен — всеки допълнителен климатик добавя ~39 BGN hardware при спестявания от ~115 BGN/сезон, което подобрява изплащането.
...
- Готов да плати 130–285 BGN за готово решение (в зависимост от броя климатици)
